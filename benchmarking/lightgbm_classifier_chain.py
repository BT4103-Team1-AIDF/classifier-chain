...
DEFAULT_RESULTS_PATH = str(PROJECT_ROOT / "outputs" / "lightgbm_classifier_chain_probabilities.csv")
RANDOM_STATE = 42
...
class ClassifierChain:
    def __init__(self, base_estimator, horizons, sample_weight_fn=None):
    ...
    def fit(self, X, y):
        ...
        for i, horizon in enumerate(self.horizons):
            ...
            if y_horizon.nunique() < 2:
                ...
                sample_weight = self.sample_weight_fn(y_horizon) if self.sample_weight_fn is not None else None
            if sample_weight is None:
                model.fit(X_extended, y_horizon)
            else:
                model.fit(X_extended, y_horizon, sample_weight=sample_weight)
...
class LightGBMWorkflow:
    ...
    @staticmethod
    def _compute_sample_weight(y_horizon):
        class_counts = y_horizon.value_counts()
        n_classes = max(len(class_counts), 1)
        raw_weights = len(y_horizon) / (n_classes * class_counts)
        clipped_weights = raw_weights.clip(upper=50.0)
        return y_horizon.map(clipped_weights).astype(float).to_numpy()

    def _suggest_params(self, trial):
        max_depth = trial.suggest_int("max_depth", 4, 8)
        max_num_leaves = max(8, min(63, 2 ** max_depth))
        return {
            "n_estimators": trial.suggest_int("n_estimators", 150, 300, step=50),
            "max_depth": max_depth,
            "learning_rate": trial.suggest_float("learning_rate", 0.01, 0.2, log=True),
            "num_leaves": trial.suggest_int("num_leaves", 8, max_num_leaves),
    ...
    def tune_hyperparameters(self, X, y, n_trials=3, validation_size=0.2, tuning_sample_size=DEFAULT_TUNING_SAMPLE_SIZE):
        stratify_base = y.iloc[:, 0] if y.shape[1] >= 1 and y.iloc[:, 0].nunique() > 1 else None
        ...
        stratify_col = y.iloc[:, 0] if y.shape[1] >= 1 and y.iloc[:, 0].nunique() > 1 else None
        ...
        def objective(trial):
            ...
            chain = ClassifierChain(
                ...
                HORIZONS,
                sample_weight_fn=self._compute_sample_weight,
            )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train and evaluate the LightGBM classifier chain.")
...
def main():
    ...
    print("\nTraining LightGBM classifier chain...")
    workflow.fit(
        X_train,
        y_train,
